...
class Player:
    ...
    def playpause(self):
        if self._obj and (self._obj.CanPlay or self._obj.CanPause):
            self._obj.PlayPause()

    def play(self):
        if self._obj and self._obj.CanPlay:
            self._obj.Play()

    def pause(self):
        if self._obj and self._obj.CanPause:
            self._obj.Pause()

    def previous(self):
        if self._obj and self._obj.CanGoPrevious:
            self._obj.Previous()

    def next(self):
        if self._obj and self._obj.CanGoNext:
            self._obj.Next()
...
class MediaWatcher:
    ...
    def _find_active_player(self):
        ...
        if players_by_status.get('playing'):
            players_by_status['playing'].sort(key=lambda x: x.owner_process.create_time() if x.owner_process else 0, reverse=True)
...
class MediaWatcherStatusBuilder:
    ...
    def _build_classes(self):
        ...
        elif player.status in ['playing', 'paused']:
